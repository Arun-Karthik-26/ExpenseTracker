...
const GenerateReport = () => {
    ...
    const fetchExpensesInRange = async () => {
        ...
        try {
            const response = await axios.get(
                `https://expensetracker-backend-s78u.onrender.com/getexpensesinrange?user=${userId}&startDate=${startDate}&endDate=${endDate}`
            );
    ...
    const fetchBudgetData = async (uid, expenses) => {
        try {
            const response = await axios.get(
                `https://expensetracker-backend-s78u.onrender.com/getbudgetdata?user=${uid}`
            );
